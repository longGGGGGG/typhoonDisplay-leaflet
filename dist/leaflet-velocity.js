...
L.Control.Velocity = L.Control.extend({
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
      ...
      htmlOut = "<strong>Location</strong>: " + `(${pos.lng.toFixed(2)}, ${pos.lat.toFixed(2)}), ` + "<strong> ".concat(this.options.velocityType, " ").concat(this.options.directionString, ": </strong> ").concat(deg.toFixed(2), "\xB0").concat(cardinal, ", <strong> ").concat(this.options.velocityType, " ").concat(this.options.speedString, ": </strong> ").concat(self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2), " ").concat(this.options.speedUnit);
    } else {
...
var Windy = function Windy(params) {
  var MIN_VELOCITY_INTENSITY = params.minVelocity || 5; // velocity at which particle intensity is minimum (m/s)

  var MAX_VELOCITY_INTENSITY = params.maxVelocity || 20; // velocity at which particle intensity is maximum (m/s)

  var VELOCITY_SCALE = (params.velocityScale || 0.009) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)

  var MAX_PARTICLE_AGE = params.particleAge || 15; // max number of frames a particle is drawn before regeneration
